...
from config.abi import MINTFUN_ABI
...
class MintFun(Minter, Logger, RequestClient):
    ...
    @gas_checker
    async def mint(self):
        mint_contracts = copy.deepcopy(Settings.MINTFUN_CONTRACTS[self.client.network.name])
        random.shuffle(mint_contracts)
        mints_count = random.randint(*copy.deepcopy(Settings.MINTFUN_MINT_COUNT))
