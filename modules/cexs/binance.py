...
from modules import CEX, Logger, CosmosClient, Client
...
from utils.tools import get_wallet_for_deposit, helper, network_handler
from config.constants import BINANCE_NETWORKS_NAME, CEX_WRAPPED_ID, TOKENS_PER_CHAIN


class Binance(CEX, Logger):
    def __init__(self, client: CosmosClient | Client):
        self.client = client
    ...
    async def deposit(self, deposit_data: tuple = None):
        ...
        if deposit_network in [42, 43]:
            raise SoftwareExceptionWithoutRetry('Binance do not support INJ and TIA deposits!')
