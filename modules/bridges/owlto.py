...
from modules.interfaces import BridgeExceptionWithoutRetry, SoftwareExceptionWithoutRetry, RequestClient, \
    SoftwareException
from config.constants import CHAIN_NAME_FROM_ID, OWLTO_CONTRACT
from config.abi import OWLTO_ABI
...
class Owlto(Bridge, Logger, RequestClient):
    ...
    async def bridge(self, bridge_data: tuple, need_check: bool = False):
        ...
        from_chain_info, to_chain_info = await self.get_chains_info(from_chain_id, to_chain_id)
        lp_config = await self.get_lp_config(to_chain_id, from_token_name)
    ...
    async def check_in(self):
        ...
        byte_data = current_time_str.encode('utf-8')
        base64_string = base64.b64encode(byte_data).decode('utf-8')

        payload = {
            'encrypt': base64_string,
            'hash': tx_hash,
