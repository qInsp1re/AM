...
from config.abi import RHINO_ABI
...
class Rhino(Bridge, Logger):
    ...
    def create_dtk(self):
        ...
        text_encoded = encode_typed_data(full_message=REGISTER_DATA)
    ...
    async def bridge(self, bridge_data: tuple, need_check: bool = False):
        (from_chain_name, to_chain_name, amount, from_token_name,
        ...
        source_chain_info = rhino_user_config['DVF']['bridgeConfigPerChain'][from_chain_name]
        ...
        await self.withdraw_from_rhino(rhino_user_config, amount, to_token_name, to_chain_name)
    ...
    async def recovery_funds(self):
        from dev_settings import Settings
        from config.constants import RHINO_CHAIN_INFO

        self.nonce, self.signature = self.get_authentication_data()
        rhino_user_config = await self.get_user_config()
        _, to_token_name = Settings.RHINO_TOKEN_NAME
        to_chain = RHINO_CHAIN_INFO[random.choice(Settings.RHINO_CHAIN_TO_NAMES)]
