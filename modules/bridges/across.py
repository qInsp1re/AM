...
from config.constants import TOKENS_PER_CHAIN, CHAIN_NAME_FROM_ID, ACROSS_CONTRACT, ACROSS_CLAIM_CONTRACTS
from config.abi import ACROSS_ABI
...
class Across(Bridge, Logger):
    ...
    async def bridge(self, bridge_data: tuple, need_check: bool = False):
        ...
        if not need_check:
            bridge_info = f'{self.client.network.name} -> {from_token_name} {CHAIN_NAME_FROM_ID[to_chain_id]}'
        ...
        min_limit, max_limit = await self.get_bridge_limits(to_chain_id, from_token_address)

        if min_limit <= amount_in_wei <= max_limit:

            if await self.check_available_routes(to_chain_id, from_token_address, to_token_address):

                pool_adress, relay_fee_pct, relay_gas_fee_total, timestamp = await self.get_bridge_fee(
                    to_chain_id, amount_in_wei, from_token_address
                )
                ...
                if from_chain_id in [324, 137, 59144, 7777777, 534352] or from_token_name == 'USDT':
