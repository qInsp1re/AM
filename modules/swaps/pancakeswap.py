...
from config.abi import PANCAKE_ABI
...
class PancakeSwap(DEX, Logger):
    def __init__(self, client):
        ...
        self.router_contract = self.client.get_contract(
            PANCAKE_CONTRACTS[self.network]['router'], PANCAKE_ABI['router']
        )
        self.quoter_contract = self.client.get_contract(
            PANCAKE_CONTRACTS[self.network]['quoter'], PANCAKE_ABI['quoter']
        )
    ...
    def get_path(self, from_token_address: str, to_token_address: str, from_token_name: str, to_token_name: str):
        ...
        }.get(self.client.network.name, 500)
