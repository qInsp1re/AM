...
from config.abi import MAVERICK_ABI
...
class Maverick(DEX, Logger):
    ...
    async def swap(self, swap_data: list):
        ...
        from_token_address, to_token_address = (
            TOKENS_PER_CHAIN[self.network][from_token_name], TOKENS_PER_CHAIN[self.network][to_token_name]
        )
    ...
    async def add_liquidity(self):
        ...
        pool_contact = self.client.get_contract(MAVERICK_CONTRACTS['pool_eth_usdc'], MAVERICK_ABI['pool'])
