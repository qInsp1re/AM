...
from config.constants import BEBOP_CONTRACTS, TOKENS_PER_CHAIN
...
class Bebop(DEX, Logger, RequestClient):
    ...
    async def swap(self, swap_data: list):
        ...
        if from_token_name == 'ETH':
            ...
            if wnative_balance < amount:
                ...
                wrap_client = await self.client.new_client(self.client.network.name)
                await wrap_eth(wrap_client, amount=amount)
            else:
                self.logger_msg(
            ...
            await self.client.check_for_approved(
                TOKENS_PER_CHAIN[self.network]['WETH'], self.router_address, amount_in_wei * 2
            )
        ...
        if required_signatures:
            permit_signature, deadline = await self.get_fee_permit_signature(required_signatures[0])
            payload["permit2"] = {
                "signature": permit_signature,
                "approvals_deadline": deadline,
                "token_addresses": [
                    f"{required_signatures[0]}"
                ],
                "token_nonces": [
                    0
                ]
        ...
        if from_token_name != 'ETH':
            unwrap_client = await self.client.new_client(self.client.network.name)
            await unwrap_eth(unwrap_client)
    ...
    async def double_swap(self):
        ...
        first_token_name, second_token_name = random.sample(Settings.BEBOP_DOUBLESWAP_FROM_TOKEN_NAMES, 2)
        ...
        if isinstance(Settings.BEBOP_DOUBLESWAP_AMOUNTS[0], str):
            start = int(Settings.BEBOP_DOUBLESWAP_AMOUNTS[0])
            end = int(Settings.BEBOP_DOUBLESWAP_AMOUNTS[1])
            ...
            start = float(Settings.BEBOP_DOUBLESWAP_AMOUNTS[0])
            end = float(Settings.BEBOP_DOUBLESWAP_AMOUNTS[0])
        ...
        if Settings.BEBOP_DOUBLESWAP_TO_TOKEN_NAME == 'ETH':
            to_token_address = ETH_MASK
        else:
            to_token_address = TOKENS_PER_CHAIN[self.network][Settings.BEBOP_DOUBLESWAP_TO_TOKEN_NAME]
        ...
        self.logger_msg(*self.client.acc_info,
                        msg=f'Double swap on bebop: {first_amount} {first_token_name} + {second_amount} '
                            f'{second_token_name} -> {Settings.BEBOP_DOUBLESWAP_TO_TOKEN_NAME}')
        ...
        if first_token_name != 'ETH':
            await self.client.check_for_approved(
                TOKENS_PER_CHAIN[self.network][first_token_name], self.router_address, first_amount_in_wei
            )

        if second_token_name != 'ETH':
            await self.client.check_for_approved(
                TOKENS_PER_CHAIN[self.network][second_token_name], self.router_address, second_amount_in_wei
            )
        ...
        elif len(required_signatures) == 1:
