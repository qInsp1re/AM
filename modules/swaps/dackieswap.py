...
from config.abi import DACKIESWAP_ABI


class DackieSwap(DEX, Logger):
    def __init__(self, client):
        self.client = client
        Logger.__init__(self)
        self.network = self.client.network.name
        self.router_contract = self.client.get_contract(
            DACKIESWAP_CONTRACTS[self.network]['router'],
            DACKIESWAP_ABI['router']
        )
        self.quoter_contract = self.client.get_contract(
            DACKIESWAP_CONTRACTS[self.network]['quoter'],
            DACKIESWAP_ABI['quoter']
        )
    ...
    async def swap(self, swap_data: list):
        ...
        self.logger_msg(
            *self.client.acc_info, msg=f'Swap on DackieSwap: {amount} {from_token_name} -> {to_token_name}')
        ...
        if from_token_name != self.client.token:
            await self.client.check_for_approved(
                from_token_address, DACKIESWAP_CONTRACTS[self.network]['router'], amount_in_wei
            )
        ...
        if to_token_name == self.client.token:
            tx_additional_data = self.router_contract.encodeABI(
                fn_name='unwrapWETH9',
                args=[
                    int(min_amount_out * 0.95),
                    self.client.address
                ]
            )
