...
from config.abi import IZUMI_ABI
...
class Izumi(DEX, Logger):
    def __init__(self, client):
        ...
        self.router_contract = self.client.get_contract(
            IZUMI_CONTRACTS[self.network]['router'], IZUMI_ABI['router']
        )
        self.quoter_contract = self.client.get_contract(
            IZUMI_CONTRACTS[self.network]['quoter'], IZUMI_ABI['quoter']
        )
    ...
    def get_path(self, from_token_address: str, to_token_address: str, from_token_name:str, to_token_name:str):
        ...
        if 'USDT' not in [from_token_name, to_token_name] and self.client.network.name != 'Manta':
