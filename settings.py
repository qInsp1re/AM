...
MINIMUM_ORGANIC_SWAP_AMOUNT = 1  # указывается в баксах, если сумма меньше, органические свапы не будут выполнены
...
DBK_BRIDGE_AMOUNT = (0.00005, 0.0001)  # применяется для бриджей в случае нехватки нативки при минте в DBK чейне
RENZO_EZETH_SWAP_AMOUNT = ('100', '100')  # Применяется для cвапов ETH -> ezETH на 1inch
...
ONEINCH_SWAP_AMOUNT = (0.009, 0.01)  # Применяется для cвапов BNB -> ZBC на 1inch. 0.001 BNB удерживается для комиссии. Минимум 0.004 BNB ~ 50 ZBC (1 бридж из Nautilus)
REFUEL_NAUTILUS_AMOUNT = ('100', '100')  # Применяется для рефьюла ZBC из BNB Chain -> Nautilus

AMBIENT_WRSETH_AMOUNT = ("45", "49")  # Применяется для свапа ETH -> wrsETH и добавления ликвидности на Ambient.
ELIXIR_AMOUNT = (0.0285, 0.03)  # Применяется для стейкинга на лендинг https://www.elixir.xyz/
ELIXIR_USDC_AMOUNT = 5   # Значение в USDC для добавления ликвидности на https://agg.elixir.xyz/ в сети Arbitrum

BEBOP_DOUBLESWAP_FROM_TOKEN_NAMES = ['USDC', 'TAIKO']  # Токены, которые будут меняться (будет выбрано 2 рандомных из списка)
BEBOP_DOUBLESWAP_AMOUNTS = ('50', '50')                # (минимум, максимум) (% или кол-во)
BEBOP_DOUBLESWAP_TO_TOKEN_NAME = 'ETH'                 # Токен, который будет получен
"""
-------------------------------------------------------CEX CONTROL------------------------------------------------------
    ...
    6 - ETH-Base                   18 - POL-Polygon         30 - USDC-Polygon (Bridged)        42 - INJ-Injective
...
FEE_SUPPORT_MIN_WITHDRAW = (0, 0)  # мин. разница между балансом аккаунта и суммой для вывода, чтобы софт сделал вывод
...
TRANSFER_COSMOS_CEXS = {
    ...
    'Injective': [2],
    'Celestia': [1],
}  # 1 - Bitget, 2 - OKX. Текущая настройка трансферит на выбранную биржу, можно указать несколько, 0 - выключить
...
OKX_DEPOSIT_DATA = [
    [(2, 3, 5, 6), ('100', '100'), 0, (0.4, 0.5)],
]

OKX_CUSTOM_DEPOSIT_1 = [
    [42, ('100', '100'), 0, (0.4, 0.5)],
]
...
-----------------------------------------------------BRIDGE CONTROL-----------------------------------------------------
    ...
    Не забудьте вставить API ключ для LayerSwap снизу. Для каждого моста поддерживается уникальная настройка
        ...
        Arbitrum                     zkSync            X Layer     
        Arbitrum Nova                Zora              Taiko
        Base                         Ethereum     
        Linea                        Avalanche     
        Manta                        BNB Chain     
        Polygon                      Metis
...
NATIVE_CHAIN_FROM_NAMES = ['Base']         # Исходящая сеть
NATIVE_CHAIN_TO_NAMES = ['Arbitrum']       # Входящая сеть
...
ACROSS_CHAIN_FROM_NAMES = ['Base']          # Исходящая сеть
ACROSS_CHAIN_TO_NAMES = ['Arbitrum']        # Входящая сеть
...
BUNGEE_CHAIN_FROM_NAMES = ['Base']          # Исходящая сеть
BUNGEE_CHAIN_TO_NAMES = ['Arbitrum']        # Входящая сеть
...
LAYERSWAP_CHAIN_FROM_NAMES = ['Base']      # Исходящая сеть
LAYERSWAP_CHAIN_TO_NAMES = ['Arbitrum']    # Входящая сеть
...
NITRO_CHAIN_FROM_NAMES = ['Base']          # Исходящая сеть
NITRO_CHAIN_TO_NAMES = ['Arbitrum']        # Входящая сеть
...
ORBITER_CHAIN_FROM_NAMES = ['Gravity']       # Исходящая сеть
ORBITER_CHAIN_TO_NAMES = ['Taiko']     # Входящая сеть
ORBITER_BRIDGE_AMOUNT = ('100', '100')      # (минимум, максимум) (% или кол-во)
ORBITER_TOKEN_NAME = 'ETH'
ORBITER_AMOUNT_LIMITER = 0, (0, 0)

'--------------------------------------------------------Owlto---------------------------------------------------------'

OWLTO_CHAIN_FROM_NAMES = ['Base']        # Исходящая сеть
OWLTO_CHAIN_TO_NAMES = ['Arbitrum']      # Входящая сеть
...
RELAY_CHAIN_FROM_NAMES = ['Base']         # Исходящая сеть
RELAY_CHAIN_TO_NAMES = ['Arbitrum']       # Входящая сеть
...
RHINO_CHAIN_FROM_NAMES = ['Base']           # Исходящая сеть
RHINO_CHAIN_TO_NAMES = ['Arbitrum']         # Входящая сеть
...
RANGO_CHAIN_FROM_NAMES = ['Base']           # Исходящая сеть
RANGO_CHAIN_TO_NAMES = ['Arbitrum']         # Входящая сеть
...
XYFINANCE_CHAIN_FROM_NAMES = ['Base']       # Исходящая сеть
XYFINANCE_CHAIN_TO_NAMES = ['Arbitrum']     # Входящая сеть
...
---------------------------------------------OMNI-CHAIN CONTROL---------------------------------------------------------
    Проверьте руками, работают ли сети на сайте. (Софт сам проверит, но зачем его напрягать?)
        ...
        Arbitrum                  Gnosis                        Polygon            Taiko                 
        Arbitrum Nova             Harmony                       Polygon zkEVM      Celestia             
        Astar                     Horizen                       Scroll             Neutron     
        Aurora                    Kava                          ShimmerEVM         Injective         
        Avalanche                 Klaytn                        Telos              Nautilus     
        BNB                       Linea                         TomoChain          Solana         
        Base                      Loot                          Tenet
        Canto                     Manta                         XPLA
        Celo                      Mantle                        Zora
        Conflux                   Meter                         opBNB
        CoreDAO                   Metis                         zkSync
        DFK                       Moonbeam                      Beam
        Ethereum                  Moonriver                     InEVM
        Fantom                    OKX                           Rarible
        Fuse                      Optimism                      Blast
        Goerli                    Orderly                       Mode
    ...
    USENEXUS_TOKENS | Выберите монеты, между которыми будут производиться свапы 
        ...
        Токены указывать в таком же порядке, как и чейны. Условно USENEXUS_CHAINS = ["Neutron", "Celestia"] и
        USENEXUS_TOKENS = ['TIA.n', 'TIA.n'] будет означать, что для чейна "Neutron" будет TIA.n, и для "Celestia" TIA.n
        Можно указывать любое количество сетей, софт будет делать бриджи в рандомную сеть из списка. 
        
        Варианты работы всех бриджей:
        
        1) Круговой бридж с заходом из сети. Указав USENEXUS_CHAINS = ["Arbitrum", (52, 53)], софт сделает бридж 
        ...
        2) Режим строгого маршрута. Указав USENEXUS_CHAINS = (1, 7, 22) и запустив модуль 1 раз, софт будет делать
        ...
        3) Режим случайных сетей. Указав USENEXUS_CHAINS = [1, 7, 22] и USENEXUS_BRIDGE_COUNT равное 1, запустив модуль
    ...
    SEARCH_CHAINS | Настройка для поиска балансов во время прогона объемов, для врапов, трансферов, апрувов
                              Укажите между какими чейнами софт будет искать балансы.
    
    JUMPER_ROUTE_TYPE | Установка своего роута для совершения транзакции, по умолчанию указаны все, софт выберет самый
                        лучший. Чтобы убрать какой либо роут, удалите цифру из списка 
...
SEARCH_CHAINS = ["Arbitrum", "BNB Chain", "Base", "Optimism"]  # Сети, в которых софт будет искать нативку для разбавляющих транз
...
SUPERFORM_CHAIN_FROM = ['Arbitrum', 'Base', 'Optimism']
SUPERFORM_CHAIN_WITHDRAW = ['Arbitrum', 'Base', 'Optimism']
SUPERFORM_VAULTS_TO = {
    "Arbitrum": ['UNjRpwsi2GF-0zFRap7nt', '5SUdXKtvBYkCyCxnLfwlM', '0mwur7ruIQk03CvTgl1t5', 'dNZCSfoPwId9TRrPYsjol'],
    "Base": ['VYKkjy_0XXzgZqeqkZKFo', 'Azq9mj0OQXjxUEFnvljUM', '-Sur6c_8I5ujr6FunDxWJ'],
    "Optimism": ['FZuZKa74BkF7AlFM7_7lo', 'T-LK2lQOYDIgNbNOim0pd', 'yJyG7wP3VdGsrd5ADzevO', 'FebJZszw5zx0HBSHe6lqK']
}
SUPERFORM_TOKEN_NAME = ['ETH', 'WETH']
SUPERFORM_AMOUNT = (0.002, 0.003)
SUPERFROM_EXCLUDE_ROUTE = 2  # 1 - LayerZero, 2 - Hyperlane, 3 - Wormhole

'-------------------------------------------------------UseNexus-------------------------------------------------------'

USENEXUS_CHAINS = ("Celestia", "Neutron", "Arbitrum", "Neutron", "Celestia")
...
MERKLY_CHAINS = ['Arbitrum', 'Base', 'Optimism']
...
RENZO_CHAINS = ['Arbitrum', 'Base', 'Optimism']
RENZO_TOKENS = ['ezETH', 'ezETH', 'ezETH']
...
NAUTILUS_CHAINS = ["Nautilus", "BNB Chain"]
...
STARGATE_CHAINS = [['Arbitrum', 'Base', 'Optimism'], ['Scroll']]
STARGATE_TOKENS = ['ETH', 'ETH', 'ETH', 'ETH']
STARGATE_AMOUNT = ('100', '100')
STARGATE_BRIDGE_COUNT = 1
STARGATE_RUN_TIMES = 1
STARGATE_AMOUNT_LIMITER = 0, (0, 0)

'-----------------------------------------------------SquidRouter------------------------------------------------------'

SQUIDROUTER_CHAINS = ['Arbitrum', 'Base', 'Optimism']
SQUIDROUTER_TOKENS = ['ETH', 'ETH', 'ETH']
...
DEBRIDGE_CHAINS = ['Arbitrum', 'Base', 'Optimism']
DEBRIDGE_TOKENS = ['ETH', 'ETH', 'ETH']
...
RANGO_CHAINS = ['Arbitrum', 'Base', 'Linea', 'Optimism']
...
JUMPER_CHAINS = ['Arbitrum', 'Base', 'Optimism']
JUMPER_TOKENS = ['ETH', 'ETH', 'ETH']
JUMPER_AMOUNT = ('100', '100')
JUMPER_BRIDGE_COUNT = 1
JUMPER_RUN_TIMES = 1
JUMPER_ROUTE_TYPE = [1, 2, 4, 5, 7, 8, 9, 10]
JUMPER_AMOUNT_LIMITER = 0, (0, 0)
JUMPER_SWAP_TOKENS = ['WETH', 'USDC']

'--------------------------------------------------------L2Pass--------------------------------------------------------'

SRC_CHAIN_L2PASS = ['Arbitrum', 'Base', 'Optimism']         # Исходящая сеть для L2Pass
DST_CHAIN_L2PASS_NFT = ["Celo", "Gnosis", "Moonbeam"]        # Входящая сеть для L2Pass Mint NFT
DST_CHAIN_L2PASS_REFUEL = {
    "Celo": (0.00001, 0.00002),  # Chain ID: (минимум, максимум) в нативном токене входящей сети (кол-во)
    "Gnosis": (0.00001, 0.00002),  # Chain ID: (минимум, максимум) в нативном токене входящей сети (кол-во)
    "Moonbeam": (0.00001, 0.00002),  # Chain ID: (минимум, максимум) в нативном токене входящей сети (кол-во)
}

'-------------------------------------------------------nogem.app------------------------------------------------------'

SRC_CHAIN_NOGEM = ['Arbitrum', 'Base', 'Optimism']             # Исходящая сеть для nogem.app
DST_CHAIN_NOGEM_NFT = ["Celo", "Gnosis", "Moonbeam"]       # Входящая сеть для nogem.app Mint NFT
...
SRC_CHAIN_MERKLY = ['Arbitrum', 'Base', 'Optimism']         # Исходящая сеть для Merkly
DST_CHAIN_MERKLY_NFT = ["Celo", "Gnosis", "Moonbeam"]     # Входящая сеть для Merkly Mint NFT
...
SRC_CHAIN_WHALE = ['Arbitrum', 'Base', 'Optimism']          # Исходящая сеть для Whale
DST_CHAIN_WHALE_NFT = ["Celo", "Gnosis", "Moonbeam"]     # Входящая сеть для Whale Mint NFT
...
SRC_CHAIN_ZERIUS = ['Arbitrum', 'Base', 'Optimism']          # Исходящая сеть для Zerius
DST_CHAIN_ZERIUS_NFT = ["Celo", "Gnosis", "Moonbeam"]      # Входящая сеть для Zerius Mint NFT
...
SRC_CHAIN_BUNGEE = ['Arbitrum', 'Base', 'Optimism']          # Исходящая сеть для Bungee
...
SRC_CHAIN_MERKLY_HYPERLANE = ['Scroll']   # Исходящая сеть для Merkly Hyperlane
DST_CHAIN_MERKLY_HYPERLANE = ["Celo", "Gnosis"]   # Входящая сеть для Merkly Hyperlane
MERKLY_HYP_TOKENS_AMOUNTS = ([2, 3], [1, 2])   # Кол-во токенов для минта и бриджа на Merkly через Hyperlane

'----------------------------------------------------Nogem Hyperlane---------------------------------------------------'

SRC_CHAIN_NOGEM_HYPERLANE = ['Arbitrum', 'Base', 'Optimism']   # Исходящая сеть для Nogem Hyperlane
DST_CHAIN_NOGEM_HYPERLANE = ["Celo", "Gnosis"]   # Входящая сеть для Nogem Hyperlane
NOGEM_HYP_TOKENS_AMOUNTS = ([2, 3], [1, 2])   # Кол-во токенов для минта и бриджа на Nogem через Hyperlane

'-------------------------------------------------------GetMint--------------------------------------------------------'

SRC_CHAIN_GETMINT_HYPERLANE = ['Arbitrum', 'Base', 'Optimism']   # Исходящая сеть для GetMint
DST_CHAIN_GETMINT_HYPERLANE = ["Celo", "Gnosis", "Polygon"]   # Входящая сеть для GetMint

'--------------------------------------------------------Womex---------------------------------------------------------'

SRC_CHAIN_WOMEX_HYPERLANE = ['Arbitrum', 'Base', 'Optimism']   # Исходящая сеть для GetMint
DST_CHAIN_WOMEX_HYPERLANE = ["Celo", "Gnosis", "Polygon"]   # Входящая сеть для GetMint
...
-----------------------------------------------------OTHER SETTINGS-----------------------------------------------------
    ...
    AMBIENT_PERCENT_RANGE | Процент покрытия цены в пуле
    
"""

AMBIENT_PERCENT_RANGE = [0.5]  # 0.5 = 0.5%, 1.0 = 1%
...
FULL_CUSTOM_SWAP_DATA1 = (['ETH', '0xFd086bC7CD5C481DCC9C85ebE478A1C0b69FCbb9'], (0.001, 0.001), "Arbitrum")
FULL_CUSTOM_SWAP_DATA2 = (['0xFd086bC7CD5C481DCC9C85ebE478A1C0b69FCbb9', 'ETH'], ('100', '100'), "Arbitrum")
FULL_CUSTOM_SWAP_DATA3 = (['0xFd086bC7CD5C481DCC9C85ebE478A1C0b69FCbb9', '0xFF970A61A04b1cA14834A43f5dE4533eBDDB5CC8'], ('100', '100'), "Arbitrum")
...
-------------------------------------------------------HYPERLANE--------------------------------------------------------            
    ...
    swap_eth_to_ezeth        # свап ETH -> ezETH в сети с большим балансом на 1inch   
    swap_ezeth_to_eth        # свап ezETH -> ETH в сети с балансом ezETH на 1inc. см. RENZO_EZETH_SWAP_AMOUNT   
    ...
    bridge_renzo             # бриджи на https://renzo.hyperlane.xyz/. См. OMNI-CHAIN CONTROLE
...
--------------------------------------------------------LANDINGS--------------------------------------------------------            
    ...
    deposit_usdc_elixir              # депозит USDC на https://agg.elixir.xyz/ в сети Arbitrum см. ELIXIR_USDC_AMOUNT
    withdraw_usdc_elixir             # вывод USDC на https://agg.elixir.xyz/ в сети Arbitrum
...
---------------------------------------------------------OTHER----------------------------------------------------------            
    ...
    add_liquidity_ambient            # добавление ликвидности в wrsETH/ETH пул на Ambient. см. AMBIENT_WRSETH_AMOUNT
    remove_liquidity_ambient         # вывод ликвидности из wrsETH/ETH пула на Ambient. Выводит 100% баланса пула
    ...
    mint_stix_pass                   # минт соответствующей NFT (STIX Launch Tournament Pass) в сети Base
    ...
    custom_rubyscore                 # аналогичный модуль сверху, только с подачей своей RPC           
    custom_check_in                  # аналогичный модуль сверху, только с подачей своей RPC       
    custom_mintfun                   # аналогичный модуль сверху, только с подачей своей RPC       
    custom_dmail                     # аналогичный модуль сверху, только с подачей своей RPC       
    custom_transfer_eth              # аналогичный модуль сверху, только с подачей своей RPC           
    custom_transfer_eth_to_myself    # аналогичный модуль сверху, только с подачей своей RPC                       
    custom_wrap_eth                  # аналогичный модуль сверху, только с подачей своей RPC       
    custom_unwrap_eth                # аналогичный модуль сверху, только с подачей своей RPC           
    custom_random_approve            # аналогичный модуль сверху, только с подачей своей RPC           
    ...
    CLASSIC_ROUTES_MODULES_USING = [
       ['custom_swap:Arbitrum'], # после знака ':' название сети для работы, название сети должно быть строго из примера
    ]
...
CLASSIC_ROUTES_BLOCKS_COUNT = [6, 6]  # Количество блоков к работе для одного аккаунта. Указывайте от 1 до 7 блоков
