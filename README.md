...
![](https://github.com/realaskaer/Scroll/blob/master/data/Annihilator_github.png)

## Общая информация

В этой машине сосредоточено все: мой и ваш опыт, все самые лучшие наработки и все актуальные проекты. 
Самый мощный инструмент в умелых руках. Автоматизирован на 95%. Софт работает в EVM, Cosmos и Solana сетях.
...
Все настройки вынесены в файл `settings.py` и `general_settings.py` Заходим в них и видим подробное описание каждого раздела.
...
* `GAS_LIMIT_MULTIPLIER` и `GAS_PRICE_MULTIPLIER` - множители газ лимита и прайса соответственно.
* `MAXIMUM_RETRY` - количество повторных попыток при ошибках в модулях
* `SLEEP_MODE`, `SLEEP_TIME_MODULES`, `SLEEP_TIME_ACCOUNTS` - включает/выключает режим сна после каждого модуля и между аккаунтами. Включив параллельный режим софта и
...
```bash
  git clone https://github.com/realaskaer/Annihilator.git
```
...
> Сначала, прочитать README, если не получилось с первого раза, прочитать инструкцию, если и она не помогла, то прошу пройти в ваш личный чат поддержки.
